...
#    Ignores Annotet extrema option. Extremas have allways a tick (at least 2 ticks are are in the scalebar).  
...
class scV(QWidget):
    def __init__(self, nC, arColo, arV, md, fileIN, outPTH, parent=None):
        ...
        creation!
        """
        super(scV, self).__init__(parent)
        self.nC     = nC
        self.arColo = arColo
        self.arV    = arV
        self.md     = md
...
#        hh = fm.height()
        ...
        if str(md.scTitle) != "":
            ...
            fm = QFontMetricsF(myFNT)
            hi = fm.height()
            wi = fm.boundingRect(QString(LT)).width()
            if LT.find("\n") >= 0:
                # multilines title
                hi = hi * (LT.count("\n")+1)
                ...
                wi = 0
                for q in L:
                    wi = max(wi, fm.boundingRect(QString(q)).width())
        #
        L = None
        if str(md.scUnits) != "":
            ...
            if LU != '':
                ...
                myP.setFont(myFNT)
                fm = QFontMetricsF(myFNT)
                hi += fm.boundingRect(QString(LU)).height() + 12
                wi = max(fm.boundingRect(QString(LU)).width(), wi)
        #
        ...
        wt = fm.boundingRect(QString("0" * wl)).width()
        #
#        vmini = ""
        if bool(md.doExtrema):
            if str(md.mini).isdigit():
                ...
                vmin = str(md.mini)
            i = fm.boundingRect(QString(vmin)).width()
            if i > wt: wt = i
        #
            v = GetL(arV[nC-1], md.txtDeci, md.bEnforce)
            i = fm.boundingRect(QString(v)).width()
            if i > wt: wt = i
        #
        ...
        for i in range(0,nC,j):
            ...
            #
            text  = (L0 + str(v))[-wl:]
            nRect = fm.boundingRect(QString(text))
            rectf = QRectF(nRect)
            mPtF.setX(float(pw - int(md.boxW) -12 -goff))
            mPtF.setY(float(y))
            rectf.moveBottomRight(mPtF)
            myP.drawText(rectf, Qt.AlignRight, QString(text))       # annotation
        ...
        if bool(md.doExtrema):
            ...
            if bTop:
                ...
                rectf.moveBottomRight(mPtF)
                myP.drawText(rectf, Qt.AlignRight, QString(text))
        ...
        if LT != '':
            ...
            myP.drawText(rectf, Qt.AlignCenter, QString(text))
        #Units
        fm = None
        if LU != '':
            myFNT = QFont()
...
class scH(QWidget):
    def __init__(self, nC, arColo, arV, md, fileIN, outPTH, parent=None):
        ...
        creation!
        """
        super(scH, self).__init__(parent)
        self.nC     = nC
        self.arColo = arColo
        self.arV    = arV
        self.md     = md
...
#        hh = fm.height()
        ...
        if str(md.scUnits) != "":
            ...
            if LU != '':
                ...
                hi += fm.boundingRect(QString(LU)).height() + 12
                wi = max(fm.boundingRect(QString(LU)).width(), wi)
        wi += fm.boundingRect(QString("WW")).width()
        wun = fm.boundingRect(QString("0")).width()
        #
        #Define necessary space to plot the scale bar with its ticks and title
        #maximum tick value length in pixels
        for i in range(nC):
...
#        L0   = " " * wl
        bTop = True
        y    = y0 + int(md.boxH)
        yoff = 13.0 + fm.boundingRect(QString("0")).height() / 2.0
        xx0  = x0 + offset
        #
        ...
        #
        if bool(md.doExtrema):
            yoff = 13 + fm.boundingRect(QString("0")).height()
            #bottom extremum
            if str(vmin).strip() != "":
                text  = QString(GetL(vmin, md.txtDeci, md.bEnforce))
                nRect = fm.boundingRect(text)
            ...
            if bTop:
                #top extremum
                text  = QString(GetL(arV[nC-2], md.txtDeci, md.bEnforce))
                nRect = fm.boundingRect(text)
                rectf = QRectF(nRect)
                x = x0 + int(md.boxW)*nC
                mPtF.setX(float(x))
...
#===============================================================================
#
class scalebarVertical(QWidget):
    def __init__(self, nC, arColo, arV, md, fileIN, outPTH, parent=None):
        """
        Create and save a vertical scale bar
        ...
        creation!
        """
        super(scalebarVertical, self).__init__(parent)
        self.nC     = nC
        self.arColo = arColo
        self.arV    = arV
        self.md     = md
    ...
    def paintEvent(self, event=None):
        ...
        if md.bAuto:
            ...
            if nC == 1:
                ticks = 1
            elif jt < int(md.boxH): ticks=nC
            else:
                ticks = int(round(float(md.boxH)*nC / jt))
                if ticks == 0: 
                    ticks = 2
        else:
            ticks = int(md.nbTicks)
            if ticks > nC: ticks = nC
        ...
        step = float(nC) / float(ticks-1)
        #
        L0 = " " * wl
        mPtF = QPointF()
        #
        for i in range(ticks):
            if i == ticks-1 or math.ceil(i*step) >=nC:
                v  = str(GetL(arV[nC], md.txtDeci, md.bEnforce))
                i0    = ht - int(md.boxH) * nC - jt
            else:
                v  = str(GetL(arV[int(round(i*step))], md.txtDeci, md.bEnforce))
                i0    = ht - int(md.boxH) * int(round(i*step)) - jt
            #
            y     = i0 + fm.height() / 2.0
            y1    = i0
            xx    = pw - int(md.boxW) -goff
...
#===============================================================================
#
class scalebarHorizontal(QWidget):
    def __init__(self, nC, arColo, arV, md, fileIN, outPTH, parent=None):
        """
        Create and save a horizontal scale bar
        ...
        creation!
        """
        super(scalebarHorizontal, self).__init__(parent)
        self.nC     = nC
        self.arColo = arColo
        self.arV    = arV
        self.md     = md
    ...
    def paintEvent(self, event=None):
        ...
        #
        pw -= wt / 2.0
        x0 = float(wt / 2.0 + wun / 2.0)
        #
        if wi > wa:  goff = float(wi - wa) / 2.0
        else:        goff = -float(pw - (int(md.boxW)*nC)) / 2.0
        x0 += goff
        #
        # Create a new QImage of the correct size and draw the scale bar on it
        myIMG = QImage(pw,ht,QImage.Format_RGB32)
        ...
        if md.bAuto:
            ...
            if nC == 1:
                ticks = 1
            elif wt < int(md.boxW): ticks=nC
            else:
                ticks = int(round(float(md.boxW)*nC / wt))
                if ticks == 0: ticks = 2
        else:
            ticks = int(md.nbTicks)
            if ticks > nC: ticks = nC
        ...
        for i in range(ticks):
            if i == ticks-1 or math.ceil(i*step)>=nC:
                ...
                v  = str(GetL(arV[int(round(i*step))], md.txtDeci, md.bEnforce))
                x  = x0 + offset + int(md.boxW) * round(i*step)
            #
            text  = QString(v)
            nRect = fm.boundingRect(text)
            rectf = QRectF(nRect)
...
def ReadTableFile(tblFile):
    ...
    if n >= len(arAll):
        ...
        if arAll[0].find("INTERPOLATION:DISCRETE") >= 0:
            return GetFromTBL(arAll)
        else:
            #Not even, abort
            return [False, "ERROR: Wrong input file \
                   (must be a colour table or a custom QML file)!", ""]
    else:
...
def GetFromTBL(ar):
    ...
    for a in ar:
        if a.find(",") >= 0:
            ...
            nC += 1
    #
    return [nC, arColo, arV]
#
#===============================================================================
#
def GetFromQML(n0, ar):
    """ From a QGIS QML file (custom colour ramp, DISCRETE algorithm) """
    n = n0 + 1
    if ar[n].strip() != "<colorRampType>DISCRETE</colorRampType>":
         return [False, "ERROR: Wrong input file (2)!", ""]
    #
    #Read colour values
    ...
    for a in ar:
        ...
        if a[0:15] == "<colorRampEntry":
            ...
            try:
                ...
                #unhandled error
                return [False, "ERROR: Wrong input file (3)!", ""]
            nC += 1
        elif i>n:
            break
    #
